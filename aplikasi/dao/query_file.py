...
APP_NAME    = f'demand-forecast-{INSTANCE_ID}'
...
async def connection_redis_async(p_decode=True):
...
def get_pool_stats():
    try:
        def _pool_info(p):
            ...
            return {
                "minconn": getattr(p, 'minconn', '?'),
                "maxconn": getattr(p, 'maxconn', '?'),
            }

        def _redis_info(client):
            if client is None:
                return "not_initialized"
            ...
            if pool is None:
                return "no_pool"
            return {
                "max_connections": getattr(pool, 'max_connections', '?'),
                "created_connections": getattr(pool, '_created_connections', '?'),
                "available_connections": len(getattr(pool, '_available_connections', [])),
                "in_use_connections": len(getattr(pool, '_in_use_connections', set())),
            }

        return {
            "write_pool":    _pool_info(_PG_POOL_WRITE),
            "read_pool":     _pool_info(_PG_POOL_READ),
            "redis_text":    _redis_info(REDIS_TEXT_CLIENT),
            "redis_limiter": _redis_info(REDIS_LIMITER_CLIENT),
        }
...
class QueryStringDb(object):
    ...
    def execute_no_commit(self, p_query, p_param, p_response="Execute without commit successfully!"):
        try:
            self._db_connection.get_cursor().execute(p_query, p_param)
            return response_json(200, "T", p_response, None)
        except psycopg2.Error as error:
            self._db_connection.rollback()
            v_msg = f"Error execute no commit: {error.pgcode}, {str(error.pgerror)}"
            return response_json(400, "F", v_msg, None)
